...
var matrixQREventId = null;

const VALIDITY_WINDOW_SECONDS = 3 * 60;
...
var BOTINFO = {
    ...
    IPADDR: Object.values(nets)
        .map((v) => v.filter((x) => !x.internal && x.family == "IPv4"))
        .flat()
        .map((v) => v.address)[0],
    STATE: BOT_OFF,
};
...
if (!fs.existsSync(puppeteer_cache)) {
    ...
    let rgx = new RegExp(
        "^linux-(?<major>\\d+)\.(?<minor>\\d+)\.(?<build>\\d+)\.(?<patch>\\d+)$",
    );
...
async function EnterCriticalSection(i, timeout = 60000) {
    dtcon.log(
        `ENTER CRITICALSECTION ${i} - Initial ATOMICS value ${Atomics.load(slock, i)}`,
    );
    var w = "ok";
    if (
        Atomics.compareExchange(slock, i, CS_UNLOCKED, CS_LOCKED) !==
        CS_UNLOCKED
    ) {
        w = Atomics.wait(slock, i, CS_LOCKED, timeout);
        dtcon.log(
            `Atomics wait of ${i} returned ${w} - value ${Atomics.load(slock, i)}`,
        );
        if (
            Atomics.compareExchange(slock, i, CS_UNLOCKED, CS_LOCKED) ==
            CS_UNLOCKED
    ...
    dtcon.log(
        `COMPLETED ENTER CRITICALSECTION ${i}.  Stored ATOMICS value ${Atomics.load(slock, i)}`,
    );
    if (w != "ok") {
...
async function LeaveCriticalSection(i) {
    dtcon.log(
        `LEAVE CRITICALSECTION ${i} - Initial ATOMICS value ${Atomics.load(slock, i)}`,
    );
    ...
    if (Atomics.compareExchange(slock, i, CS_LOCKED, CS_LOCKED) == CS_LOCKED) {
        dtcon.log(
            `CLEARING CRITICALSECTION.  BEFORE CLEARING, ATOMICS value was ${Atomics.load(slock, i)}`,
        );
...
const client = new Client({
    ...
    ...(BOTCONFIG?.WAVERSION && {
        ...
        webVersionCache: {
            type: "remote",
            remotePath:
                "https://raw.githubusercontent.com/wppconnect-team/wa-version/refs/heads/main/html/{version}-alpha.html",
        },
    }),
});
...
async function clientConnected() {
    try {
        ...
        dtcon.error(
            `clientConnected: Failed to get state for client:\n${e?.name}: ${e?.message}\n${JSON.stringify(e?.cause, null, 2)}`,
        );
        return false;
    }
}
...
client.on(Events.DISCONNECTED, async (reason) => {
    ...
    try {
        ...
        dtcon.error(
            `DISCONNECTED: Failed to get state for client:\n${e?.name}: ${e?.message}\n${JSON.stringify(e?.cause, null, 2)}`,
        );
    }
    ...
    try {
        ...
        dtcon.error(
            `Failed to destroy client while handling disconnected event:\n${e?.name}: ${e?.message}\n${JSON.stringify(e?.cause, null, 2)}`,
        );
    }
...
async function client_ready() {
    ...
    if (!(await clientConnected())) {
        dtcon.error(
            "READY event: client is not connected - skip returning event",
        );
        return;
    }
    ...
    const prevwweb = fs.existsSync(prevwwebfs)
        ? requireUncached(prevwwebfs)
        : { version: "" };
    ...
    if (version != prevwweb.version) {
        if (prevwweb.version) {
            messages.push(
                `WhatsApp Web version has been updated from old version ${prevwweb.version}`,
            );
        }
        if (version !== "UNKNOWN") {
            fs.writeFileSync(
    ...
    if (!first_ready_received) {
        ...
        for (let [software, _] of Object.entries(packageInfo.dependencies)) {
            let installedVersion =
                installedInfo.packages[`node_modules/${software}`].version;
            let resolved =
                installedInfo.packages[`node_modules/${software}`].resolved;
        ...
        for (const chtype of Object.keys(chromium_versions)) {
            if (chromium_versions[chtype].length == 0) {
                        ...
                        (active_chromium_version.major != c.version.major
                            ...
                            : active_chromium_version.minor != c.version.minor
                              ...
                              : active_chromium_version.build != c.version.build
                                ? active_chromium_version.build >
                                  c.version.build
                                : active_chromium_version.patch >
                                  c.version.patch);
                    outdated_versions ||= outdated;

                    messages.push(
                        `  ${outdated ? asterisk : ""}${c.version.major}.${c.version.minor}.${c.version.build}.${c.version.patch}`,
                    );
                    if (outdated) {
                        let rmfolder = path.join(
                            puppeteer_cache,
                            chtype,
                            c.name,
                        );
                        messages.push(
                            `    Outdated folder to be removed: ${rmfolder}`,
                        );
                        fs.rmSync(rmfolder, { recursive: true }, (err) => {
                            if (err) {
                                messages.push(
                                    `    Error while removing ${rmfolder}: ${JSON.stringify(err)}`,
                                );
                            }
                        });
                    }
                ...
                if (outdated_versions) {
                    messages.push(
                        `  Outdated versions marked with ${asterisk} were removed from ${puppeteer_cache}${chtype}`,
                    );
                }
            }
        }
...
client.on(Events.READY, async () => {
    try {
        ...
        dtcon.error(
            `READY: Failed to get state for client:\n${e?.name}: ${e?.message}\n${JSON.stringify(e?.cause, null, 2)}`,
        );
    }
    if (!clientReadyTimeout) {
    ...
    try {
        ...
        if (msg.body.match(/^[!\/]/)) {
            if (BOTINFO.STATE == BOT_ACTIVE) {
                ...
                !chat.isGroup &&
                msg.body.trim() == `!wake ${BOTINFO.HOSTNAME}`
            ) {
...
async function monitorClient() {
    ...
    if (state == null) {
        ...
        if (!clientStartTimeoutObject) {
            ...
            dtcon.log(
                `monitorClient: Client not connected - start timer to start client in ${clientStartTime} seconds`,
            );
            clientStartTimeoutObject = setTimeout(
                startClient,
        ...
        if (clientStartTimeoutObject) {
            // Is connected - kill timer to initialize client
            dtcon.log(
                "monitorClient: Client present - clearing timer to start client",
            );
            clearTimeout(clientStartTimeoutObject);
            clientStartTimeoutObject = null;
        }
...
function isTimestampValid(timestamp) {
    ...
    return (
        Math.abs(nowInSeconds - parseInt(timestamp)) <= VALIDITY_WINDOW_SECONDS
    );
}
...
async function session_expired(key, value) {
    dtcon.log(
        `@@@@@ Session expired for ${key}, started at ${gentsdate(value)}`,
    );
...
async function session_deleted(key, value) {
    dtcon.log(
        `@@@@@ Session deleted for ${key}, started at ${gentsdate(value)}`,
    );
    dtcon.log(`@@@@@ Leaving Critical Section 0`);
    await LeaveCriticalSection(0);
}
...
async function nonce_set(key, value) {
    dtcon.log(
        `@@@@@ Nonce saved for ${key}, timestamp ${gentsdate(value * 1000)}`,
    );
}

async function nonce_expired(key, value) {
    dtcon.log(
        `@@@@@ Nonce expired for ${key}, timestamp ${gentsdate(value * 1000)}`,
    );
}
...
const server = https.createServer(serverOptions, async (req, res) => {
    ...
    let suffix = "";
    dtcon.log(
        `#### Server Connection start time: ${gentsdate(connectStartTime)}`,
    );
    ...
    if (forwardedFor) {
        ...
        clientPort =
            req.headers?.["x-forwarded-port"] ?? "No forwarded port determined";
    }
    ...
    dtcon.log(
        `Client connection from ${clientIp} : ${clientPort} -- ${suffix}`,
    );
    ...
    if (req.method == "GET") {
        ...
        dtcon.log(
            `Server handling for ${req.socket?.encrypted ? "encrypted" : "unencrypted"} socket`,
        );
        ...
        req.on("data", function (chunk) {
            ...
            if (Date.now() > changeTimeout) {
                ...
                dtcon.log(
                    `HTTP Server data: Set new socket timeout: ${resTimeout}`,
                );
            }
        });
        ...
        req.on("end", async function () {
            ...
            try {
                ...
                if (headers["content-type"] != "application/json") {
                    let errmsg =
                        "Non-json content-type" + headers["content-type"];
                    throw new Error(errmsg);
                }
                ...
                if (sessionCache.has(Xsession)) {
                    throw new Error(
                        `Session ${Xsession} already exist from earlier connection`,
                    );
                }
                ...
                dtcon.log(
                    `@@@@ Valid signature for packet at ${gentsdate(parseInt(Xtimestamp) * 1000)} with nonce ${Xnonce}`,
                );
                ...
                if (url == "/SENDMESSAGE") {
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected in - /SENDMESSAGE, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                    ...
                    if ("Phone" in obj) {
                        ...
                        response =
                            "ERROR - Illegitimate SENDMESSAGE contents - no Phone nor Group field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    while (!isConnected && count > 0) {
                        ...
                        if (Date.now() > changeTimeout) {
                            ...
                            changeTimeout = changeTimeout + 10000;
                            dtcon.log(
                                `SENDMESSAGE: Set new socket timeout: ${resTimeout}`,
                            );
                        }
                    ...
                    if (isConnected) {
                        ...
                        let msgstatus = await client.sendMessage(
                            chatId,
                            obj.Message,
                            msgoption,
                        );
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected - /SENDMEDIA, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                        throw new Error("Client is not connected");
                    }
                    let number;
                    ...
                    if ("Phone" in obj) {
                        ...
                        response =
                            "ERROR - Illegitimate SENDMEDIA contents - no Phone nor Group field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    while (!isConnected && count > 0) {
                        ...
                        if (Date.now() > changeTimeout) {
                            ...
                            changeTimeout = changeTimeout + 10000;
                            dtcon.log(
                                `SENDMEDIA: Set new socket timeout: ${resTimeout}`,
                            );
                        }
                        isConnected = await clientConnected();
                    }
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected - /SENDCONTACT, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                        throw new Error("Client is not connected");
                    }
                    let number;
                    ...
                    if ("Phone" in obj) {
                        ...
                        response =
                            "ERROR - Illegitimate SENDCONTACT contents - no Phone nor Group field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    while (!isConnected && count > 0) {
                        ...
                        if (Date.now() > changeTimeout) {
                            ...
                            changeTimeout = changeTimeout + 10000;
                            dtcon.log(
                                `SENDCONTACT: Set new socket timeout: ${resTimeout}`,
                            );
                        }
                        isConnected = await clientConnected();
                    }
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected - /SENDPOLL, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                        throw new Error("Client is not connected");
                    }
                    dtcon.log(
                        "Sending poll to " +
                            ...
                            obj.Poll.pollName +
                            " ; poll options: " +
                            obj.Poll.pollOptions.map((p) => p.name).join("##"),
                    );
                    ...
                    if ("Phone" in obj) {
                        ...
                        response =
                            "ERROR - Illegitimate SENDPOLL contents - no Phone nor Group field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    while (!isConnected && count > 0) {
                        ...
                        if (Date.now() > changeTimeout) {
                            ...
                            changeTimeout = changeTimeout + 10000;
                            dtcon.log(
                                `SENDPOLL: Set new socket timeout: ${resTimeout}`,
                            );
                        }
                        isConnected = await clientConnected();
                    }
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected - /GROUPMEMBERS, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                        throw new Error("Client is not connected");
                    }
                    ...
                    if ((!"Group") in obj) {
                        response =
                            "ERROR - Illegitimate GROUPMEMBERS contents - no Group field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    while (!isConnected && count > 0) {
                        ...
                        if (Date.now() > changeTimeout) {
                            ...
                            changeTimeout = changeTimeout + 10000;
                            dtcon.log(
                                `GROUPMEMBERS: Set new socket timeout: ${resTimeout}`,
                            );
                        }
                        isConnected = await clientConnected();
                    }
                    ...
                    if (isConnected) {
                        ...
                        if (chat && chat.isGroup) {
                            ...
                            dtcon.log(
                                "found " + grpmembers.length + " members",
                            );
                    ...
                    if (!clientIsConnected) {
                        dtcon.error(
                            `Client is not connected - /SENDCOMMAND, jsonmsg = ${JSON.stringify(obj, null, 2)}`,
                        );
                        throw new Error("Client is not connected");
                    }
                    if ((!"Command") in obj) {
                        response =
                            "ERROR - Illegitimate COMMAND contents - no Command field";
                        res.setHeader("Content-Type", "text/plain");
                        dtcon.error(response);
                        return;
                    }
                    ...
                    if (obj.Command == "ping") {
                        ...
                        let totalidle = cpus.reduce(
                            (psum, cpu) => psum + cpu.times.idle,
                            0,
                        );
                        ...
                        let memusage =
                            100 * (1.0 - os.freemem() / os.totalmem());
                        let pongobj = {
                            STATE: BOTINFO.STATE,
                            clientstate: state,
                        ...
                        dtcon.log(
                            `GROUPINFO: Current socket timeout: ${resTimeout}`,
                        );
                        ...
                        chats = chats.filter(
                            (c) =>
                                c.isGroup &&
                                !c.groupMetadata.isParentGroup &&
                                !c.groupMetadata.announce,
                        );
                        res.setHeader("Content-Type", "application/json");
                        for (const chat of chats) {
                            dtcon.log(
                                `GROUPINFO: Collecting info for group chat ${chat.name}`,
                            );
                            let grpinfo = {};
                            ...
                            if (Date.now() > changeTimeout) {
                                ...
                                changeTimeout = changeTimeout + 10000;
                                dtcon.log(
                                    `GROUPINFO: Set new socket timeout: ${resTimeout}`,
                                );
                            }
                        }
                        ...
                        if (chat) {
                            ...
                            let messages =
                                await chat.fetchMessages(searchOptions);
                            response = JSON.stringify(messages);
                            res.setHeader("Content-Type", "text/plain");
                        } else {
                            dtcon.log(
                                `Could not find chat ${obj.Parameters.name}`,
                            );
                        ...
                        if (contacts?.length) {
                            let xct = contacts.filter(
                                (c) => c.id.server == "c.us" && c.isMyContact,
                            );
                        ...
                        let resChatId =
                            await client.saveOrEditAddressbookContact(
                                obj.Parameters.phoneNumber,
                                obj.Parameters.firstName,
                                obj.Parameters.lastName,
                                true,
                            );
                        return;
                ...
                if (sessionStart) {
                    dtcon.log(
                        `Critical Section entered for session ${Xsession}`,
                    );
                ...
                dtcon.log(
                    `#### Server Connection end time: ${gentsdate(endConnectTime)}`,
                );
                dtcon.log(
                    `#### Server Connection elapsed time: ${endConnectTime - connectStartTime}`,
                );
            }
        });
    } else {
        dtcon.error(
            `HTTP Server warning: Unhandled method ${req.method}: ${JSON.stringify(req, null, 2)}`,
        );
    }
});
...
server.on("clientError", (err, socket) => {
    if (err.code === "ECONNRESET" || !socket.writable) {
        dtcon.error(
            `Returning from HTTP Server clientError: ${err?.code ?? "NoErrorCode"}`,
        );
        return;
    }
    dtcon.error(
        `Close HTTP Server due to clientError: ${err?.code ?? "NoErrorCode"}`,
    );
...
async function cmd_to_host(
    ...
    try {
        ...
        if (WEBAPPSTATE_OK) {
            let cmd_promise = promise_cmd_to_host(
                number,
                contents,
                groups,
                waevent,
